...
import com.ivandjoh.springjwt.config.SecurityConfig;
...
class HomeControllerTest {
    ...
    void tokenWithBasicThenGetToken() throws Exception {
        MvcResult result = this.mvc.perform(post("/token").with(httpBasic("ivandjoh", "password"))).andExpect(status().isOk()).andReturn();
    ...
    public void rootWithBasicStatusIsUnauthorized() throws Exception {
        this.mvc.perform(get("/").with(httpBasic("ivandjosh", "password"))).andExpect(status().isUnauthorized());
    }
